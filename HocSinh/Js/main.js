...
function add_taga_i(){
    ...
    for(var i=0;i<check_section_menu.length-2;i++){
        if(this.childNodes[1].innerHTML == check_section_menu[i].innerHTML){
            ...
            check_section_menu[i].parentNode.parentNode.parentNode.classList.remove("infor_hidden");
        }
    }
}
...
for(var i=0;i<todam.length;i++){
    todam[i].onclick = function(){
        if(this.className.search('todam') >= 0)
            this.classList.remove('todam');
        else
            this.classList.add('todam');
    }
}

function todam_hang(a){
    if(a.className.search('todam') >= 0)
        a.classList.remove('todam');
    else
        a.classList.add('todam');
}
...
function search(search_php,search_column,search_infor){
    ...
    request.open("get","" + search_php + "?col=" + search_column + "&inf=" + search_infor);
